...
class Report:
    ...
    def generate_report(self, with_cases: bool):
        ...
        insertion_dir = {
            ...
            "cases": self._generate_cases() if with_cases else "",
        }
        ...
        try:
            ...
            raise Exception(f"Report could not be rendered due to Error: {e}")
    ...
    def _get_distr_section(self) -> str:
        ...
        if "ipynb" in self.arguments["filetype"]:
            ...
            if self.code_only:
                distr_section = distr_section.replace("--#",
                                                      r"Given that, for the current submissions, markdown can be considered irrelevant in the context of measuring plagiarism, a dedicated figure is provided below for the assessment of code similarity (Fig. \ref{fig:f2}). --#")
            else:
                distr_section = distr_section.replace(r"--#",
                                                      r"To enhance clarity and visual distinction, an independent figure focusing solely on the code is presented below (Fig. \ref{fig:f2}). --#")
...
\end{figure}\\"""
    ...
    def _generate_cases(self) -> str:
        ...
        for _, case in tqdm(selection.iterrows()):
            case_string += r"\subsection*{\textbf{Case}: " + case["Submission 1"] + " and " + case[
                "Submission 2"] + r"}\\"
            code1, code2 = case["Code_Slices"]
            code1, code2 = __format_code(code1), __format_code(code2)
            case_string += r"\textbf{" + case[
                "Submission 1"] + r"}\\ \begin{python}" + "\n" + code1 + "\n" + r"\end{python}\\"
            case_string += r"\textbf{" + case[
                "Submission 2"] + r"}\\ \begin{python}" + "\n" + code2 + "\n" + r"\end{python}\newpage"
